...
namespace GoogleKeepTestCases.TestCases
{
    ...
    public class NoteTestCases
    {
      ...
      //  private readonly IFormFile formFile;
        ...
        {
            var response = noteController.NotesLabelCollaborator();
            Assert.IsNotType<BadRequestResult>(response);
        }
        ...
        [Fact]
        public void NotBadResult_ImageUpload()
        {
            var response = noteController.ImageUpload(file, 4002);
            Assert.IsNotType<BadRequestResult>(response);
        }

        [Fact]
        public void NotOkResult_ImageUpload()
        {
            var response = noteController.ImageUpload(file, 4002);
            Assert.IsNotType<OkObjectResult>(response);
        }
        ...
        [Fact]
        public void NotOkResult_AddCollaborator()
        {
            NoteController note = new NoteController(this.notesBL);
            var model = new ShowCollaboratorModel()
            {
                ReceiverId = 2
            };
            var response = note.AddCollaborator(5, model);
